...
from d2ix.postprocess import create_timeseries_df, create_barplot, \
    create_plotdata_df, extract_synonyms_colors
from d2ix.preprocess import process_demand, process_base_techs, \
    process_spec_techs, process_spatial_locations, process_units, \
    process_lvl_spatial, process_map_spatial_hierarchy, process_level
from d2ix.sets import add_sets, extract_sets, set_frame_list, set_order
from d2ix.technology import add_technology, \
    add_reliability_flexibility_parameter, create_renewable_potential, \
    change_emission_factor
from d2ix.util import model_data_yml, YAMLd2ix, check_input_data, \
    setup_logging
...
class MessageInterface(object):
    ...
    def _create_logger(self, verbose):
        setup_logging(path=_LOG_CONFIG_FILE,
    ...
    def Platform(db_config):
        return ix.Platform(dbprops=db_config.get('dbprops'),
    ...
    def Scenario(self, model, scen, version=None, annotation=None,
        ...
        if self._local_db:
            self.open_db()
        return message_ix.Scenario(self._mp, model, scen, version, annotation,
                                   cache)
...
class DBInterface(MessageInterface):
    ...
    def model2db(self):
        ...
        if self.model_type == 'new':
            ...
            units = set([self.data['units'][p]['unit'] for p in
                         self.data['units'].keys()])
            ...
            _sets = {k: v for k, v in self.model_par.items() if
                     k in self.scenario.set_list()}
        ...
        _pars = {k: v for k, v in self.model_par.items() if
                 k in self.scenario.par_list()}
    ...
    def pull_results(self, model, scen, version):
        logger.info(f'Load results for model: \'{model}\', '
                    f'scenario: \'{scen}\', version: \'{version}\' ')
...
class Model(DBInterface):
    ...
    def __init__(self, model=None, scen=None, annotation=None, base_xls=None,
                 manual_parameter_xls=None, historical_data=None,
                 historical_range_year=None, first_historical_year=None,
                 model_range_year=None, first_model_year=None,
                 last_model_year=None, run_config=None,
                 verbose=False, yaml_export=True):
    ...
    def _create_year_vectors(self):
        ...
        self.active_years = list(
            range(self.first_model_year, self.last_model_year + 1,
                  self.model_range_year))
        if self.historical_data:
            self.historical_years = list(
                range(self.first_historical_year,
                      self.first_model_year - self.model_range_year + 1,
    ...
    def _calc_duration_period(self):
        ...
        self.duration_period.update({_years[i + 1]: _years[i + 1] - _years[i]
                                     for i in range(0, len(_years) - 1)})
        ...
        _d_p_sum.update(
            {_years[i + 1]: sum(_tmp[0:i]) + self.duration_period[_years[0]]
             for i in range(1, len(_years) - 1)})

        df = pd.DataFrame(columns=self.year_vector[1:],
                          index=self.year_vector[1:-1])

        idx = list(df.index)
        for y2 in df.columns:
            df.loc[:, y2] = [int(_d_p_sum[y2] - _d_p_sum[y1]) if y1 < y2 else 0
                             for y1 in idx]
    ...
    def _load_raw_input_data(self):
        logger.info(f'Load model input data from: '
                    f'\'{self.config["base_xls"]}\'')
        ...
        if p.exists():
            ...
            logger.info(f'Load model input data from: '
                        f'\'{self.config["manual_parameter_xls"]}\'')
    ...
    def _create_model(self):
        ...
        if self.manual_input:
            logger.info(f'Create parameters from: '
                        f'\'{self.config["manual_parameter_xls"]}\'')
            self.model_par.update(
                add_parameter_manual(self.raw_data['manual_input']))
        ...
        for loc in self.data['locations'].keys():
            self.model_par.update(
                add_technology(self.data, self.model_par,
                               self.first_model_year, self.active_years,
                               self.historical_years, self.duration_period_sum,
                               loc, par='technology'))
        ...
        for loc in sorted(self.data['demand'].keys()):
            self.model_par.update(add_demand(self.data, self.model_par,
                                             loc))
            if self.ENABLE_SLACK_TECHS is True:
                self.model_par.update(
                    add_technology(self.data, self.model_par,
                                   self.first_model_year, self.active_years,
                                   self.historical_years,
                                   self.duration_period_sum, loc, par='demand',
                                   slack=True))

        # add rel and flex parameter
        if 'rel_and_flex' in self.raw_data['base_input'].keys():
            logger.info(f'Create parameters from: \''
                        f'{self.config["base_xls"]}\' - \'rel_and_flex\'')
            self.model_par.update(
                add_reliability_flexibility_parameter(self.data,
                                                      self.model_par,
                                                      self.raw_data))
        ...
        if 'renewable_potential' in self.raw_data['base_input'].keys():
            logger.info(f'Create parameters from: \'{self.config["base_xls"]}'
                        f'\' - \'renewable_potential\'')
            self.model_par.update(
                create_renewable_potential(self.raw_data, self.data,
                                           self.active_years))

        # change emission factor
        if 'emissions' in self.raw_data['base_input'].keys():
            logger.info(f'Change emission factor from: \''
                        f'{self.config["base_xls"]}\' - \'emissions\'')
            self.model_par.update(
                change_emission_factor(self.raw_data, self.model_par))

        # add sets
        logger.info(f'Create sets from: \'{self.config["base_xls"]}\' and \''
                    f'{self.config["manual_parameter_xls"]}\'')
        self.model_par.update(extract_sets(self.scenario, self.model_par))
        self.model_par.update(
            add_sets(self.data, self.model_par, self.first_model_year))
...
class ModifyModel(DBInterface):
    def __init__(self, run_config=None, model=None, scen=None,
                 xls_dir='scen2xls', file_name='data.xlsx', verbose=False,
                 yaml_export=True):
        super().__init__(run_config, verbose, yaml_export)
        self.model = model
        self.scen = scen
    ...
    def get_model_pars(self, version=None):
        ...
        self.model_par.update(
            {par: self.scenario.par(par) for par in self.scenario.par_list()})
        self.model_par.update(
            {sets: self.scenario.set(sets) for sets in
             self.scenario.set_list()})
    ...
    def xls2model(self, annotation=None):
        ...
        self.version = 'new'
        self.annotation = annotation
        self.scenario = self.Scenario(self.model, self.scen, self.version,
                                      self.annotation)


class PostProcess(DBInterface):

    def __init__(self, run_config, model, scen, version, base_xls=None,
                 verbose=False):
        super().__init__(run_config, verbose)
        self.model = model
        self.scen = scen
    ...
    def _get_synonyms_colors(self):
        ...
        self.raw_data['spec_techs'] = pd.read_excel(self.base_xls,
                                                    sheet_name='spec_techs')
        _tmp = self.raw_data['spec_techs'].get(
            ['technology', 'postprocess_color', 'postprocess_synonym'])
        if isinstance(_tmp, pd.DataFrame):
            if not _tmp.empty:
                _tmp = _tmp.rename(columns={k: k.replace('postprocess_', '')
                                            for k in _tmp.columns})
